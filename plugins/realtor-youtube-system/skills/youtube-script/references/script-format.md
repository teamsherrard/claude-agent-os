...
- Match the runtime: **~140 spoken words per minute**. A 10–12 min video ≈ **1,400–1,700 words — no more.**
...
[HOOK — 0:00]
{1–3 lines: name the situation/fear, promise the payoff. Use the idea's hook.}

[EARLY WARM CTA — ~0:20]
{Friendly, non-salesy invite to connect — captures leads early. See script-framework.md.}

[INTRO — ~0:35]
>> on-screen: {agent name | market}
{Who the agent is + what this video delivers. Brief, credible.}

[SECTION 1 — {label} — ~MM:SS]
{Concise spoken lines; talking-point bullets for lists. Real specifics + one story where natural.}
{Any stat → with a (source, date) tag. Pattern-interrupt cue in [brackets].}

[SECTION 2 …]  [SECTION 3 …]   (3–5 sections — enough to deliver value, not to pad)

[SOFT MID CTA — ~halfway]
{One light, value-first reminder tied to the content — "the full checklist's linked below." Not salesy.}

[REASSURANCE / PAYOFF — ~MM:SS]
{Address the core fear; deliver the promise.}

[FINAL CTA — ~MM:SS]
{Specific next step + a nudge to the end screen (next video / playlist / subscribe).}
...
## CTAs — sprinkled, never salesy
Three light touches only: the **Early Warm CTA** (~0:20), one **Soft Mid CTA** (~halfway), and the
**Final CTA**. Each is value-first and in the agent's voice — never pushy, never more than three.

## Short-form cut (30–45s)
Hook → one core point → CTA → caption + hashtags (#Shorts #YouTubeShorts + 2–4 niche + 1–2 trending).
