...
6. **B-roll** (**pass 3** — import the clips, then ONE placement call) — `editor-broll`, on the words, **full-cover, never over the hook/face/CTA, and NEVER the same clip twice in a row**. **HARD CAP: MAXIMUM 3 clips on a reel** — not more; over-b-roll drains credits and clutters. If the agent wants more, they add it by hand (free). Horizontal b-roll → split-screen or clean full-bleed, never over-zoomed/floating (`${CLAUDE_PLUGIN_ROOT}/shared/layouts.md`).
