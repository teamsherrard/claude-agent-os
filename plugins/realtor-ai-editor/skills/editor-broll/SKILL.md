...
- **Placement & amount (scale to length)** — 2.5–5s each, on the exact words; **full-cover, never a partial clip floating over the face**; never cover the hook, the face, or the CTA. **A short-form reel (≤ ~2 min) gets a MAXIMUM of 3 cutaways TOTAL — never more** (over-b-roll drains credits, clutters, and buries the agent's face). **Long-form scales with length: ~1 cutaway per ~45–60 SECONDS of finished video — a ~10-min video ≈ 10–13 clips, a ~15-min ≈ 15–20** (~10–15% of screen time; corrected after a live test where the old 1-per-1.5–2-min rule left a 12-min video at under 4% coverage and reading flat) (on the key lines + section changes), curated, never wall-to-wall. **If the agent wants more, they add it by hand in Descript (free). About to exceed 3 on a reel? STOP and cut the weakest instead of adding.**
