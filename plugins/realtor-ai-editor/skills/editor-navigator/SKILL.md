...
Both video editors can be installed side by side. Read `default_engine` in `~/realtor-brain/editor/config.json`. If it says `riverside` and the agent did not name Descript, hand to the AI Editing Studio's `studio-navigator` and stop. If it says `descript`, or the agent names Descript, or the video is a Descript project, carry on here.
