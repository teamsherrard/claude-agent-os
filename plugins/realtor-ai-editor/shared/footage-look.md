# Footage look & treating people well

How the footage — and the people in it — should look. Subtle and flattering always beats "punchy."

## Colour grade — ALWAYS applied, subtle and natural only

- **A colour grade is REQUIRED on every video — never deliver ungraded, flat footage.** Even when the lift is tiny, apply one: ungraded raw footage reads cheap and unfinished. The mistake to avoid is BOTH directions — no grade at all, AND an over-cooked grade.
- A gentle lift is fine: slight contrast, a touch of warmth, balanced brightness. It should look crisp, never "filtered."
- **Hard guardrails (these have gone wrong before):** never over-saturate, never over-brighten, never crush the blacks, never anything neon / harsh / pixelated. Skin tones stay natural and flattering — never orange, never washed out.
- **Bare minimum is the target** — tiny tweaks only. Over-saturation / over-warmth is what makes skin look pale, orange, or yellow (the recurring complaint). When in doubt, do less — and offer to dial it back.

## Treat people flatteringly (this matters)

Many agents — **especially women in the cohort** — are self-conscious on camera about their face, skin, hair, and makeup. Default to making them look their best:

- **Gentle zooms only (~1.05–1.08×). NEVER a tight push-in on someone's face.**
- Keep the face well-framed with headroom; don't crop in close.
- Natural, flattering skin; never harsh brightness or contrast on a face.
- When unsure, prioritise making the person look good over a "punchier" edit.
- Never comment on or try to "fix" someone's appearance — just frame and grade them kindly.

## Zooms & framing — never expose the edge

- Every clip must FILL the frame (cover, not "fit") — no black bars or letterboxing anywhere, especially the first second.
- Zooms / punch-ins are **zoom-IN only** — stay at or above the full-fill scale (≥100%). Never scale below fill; never pan footage off-frame.
- **A position offset must stay small relative to the zoom.** A shift bigger than the zoom's headroom pushes past the source edge and creates a black bar (a 7.5% vertical offset at 1.05–1.08× zoom exposed the top edge in testing). Keep footage centred unless there's a reason, and stay within the safe range.
- After any zoom or reframe, verify there are **zero canvas gaps** (no black showing) across the whole timeline before calling it done.
