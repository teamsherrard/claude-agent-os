...
- **Real measured cost (a 15-min YouTube video edited end-to-end, cut to 12 min):** **~207 AI credits + ~15 media minutes**, 8 paid passes, ~60 min wall clock — versus **490 credits** for the same kind of video under the old undisciplined flow (~2.4× cheaper). Where it went: the **energy pass is the most expensive single pass (~60 credits** — punch-ins + transitions + SFX are many discrete placements), the text/cards pass ~33, the text-fix round ~44; cuts, audio, and grade are cheap (~9–23 each). **Plan on ~200–250 credits for a 10–20 min long-form**, and quote that to the agent up front. Media minutes ≈ the video's length, once — never re-import.
