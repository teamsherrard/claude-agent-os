...
5. **Placement & amount (HARD CAP — protects the agent's credits)** — 2.5–5s each, on the exact words; **full-cover cutaways are the DEFAULT — never a partial clip floating over the speaker's face.** (The one sanctioned exception is a deliberate **split-screen** on a vertical reel when horizontal b-roll would be destroyed by cropping — that's a designed two-band layout, not a floating clip. See `${CLAUDE_PLUGIN_ROOT}/shared/layouts.md`.); never over the hook or CTA. **Amount — scale to length:** a short-form reel (≤ ~2 min) gets a **MAXIMUM of 3 B-roll clips — never more.** **Long-form scales with length: roughly 1 cutaway per ~45–60 SECONDS of finished video — so a ~10-min video ≈ 10–13 clips, a ~15-min ≈ 15–20** (target roughly **10–15% of screen time** on B-roll). ⚠️ **Corrected from live testing:** the old rule (1 per 1.5–2 min) produced only **7 clips / 28 seconds / under 4% coverage** on a real 12-minute YouTube video — it read as a flat, static talking head and the agent immediately asked for more. Long-form needs materially more B-roll than short-form, not less. Place them ALL in one batched pass to stay credit-sane (placed on the key lines + section changes), curated, never wall-to-wall (no short-form-style hard ceiling, but don't carpet the whole video). **If the agent wants more B-roll than this, they add it themselves by hand in Descript (free — no AI credits).** Do not exceed the cap to be "thorough" — over-B-roll drains credits, clutters, and buries the agent's face. About to exceed 3 on a reel? STOP and cut the weakest; don't add. (See `${CLAUDE_PLUGIN_ROOT}/shared/credit-tiers.md` — the 80/20 rule.)
