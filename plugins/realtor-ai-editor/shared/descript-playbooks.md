...
- `import_media` — bring in the video and B-roll. **Import by URL** (a direct link, **Google Drive**, or **Dropbox**) whenever possible. Returns a `job_id` + `project_url`. **API SHAPE (changed — get this right or the edit strands on step one):** media goes in an **`add_media` map** of `name → {url}` (e.g. `{"main_video": {"url": "https://…"}}`) — `add_media` is REQUIRED. And on a NEW project you must ALSO pass **`add_compositions`** with a clip referencing that media name, or the file lands in the media library with an **EMPTY timeline** (`duration: 0`) and nothing to edit. To put media on an EXISTING composition use `update_compositions` (`composition_id` + `append_clips`) — but note `append_clips` can only reference media in the same call's `add_media`, so recovering an empty timeline without re-downloading means asking `prompt_project_agent` to place the already-imported media (~10 credits) instead of re-importing (which re-burns the whole video's media minutes).
...
- `publish_project` — render/export → returns `share_url` + `download_url`. **Resolution is a required decision, not a default — you must pass it.** Descript will publish anywhere from 480p to 4K, and a 480p listing video is a client-facing disaster. **Always `1080p`** — for long-form 16:9 and for vertical alike (on a 9:16 composition, 1080p renders 1080×1920, which is exactly right for every platform). **NEVER below 1080p** for anything the agent or a client will see. **4K only if** the source is genuinely 4K AND the agent asks AND it's a hero long-form — warn them first: renders take much longer and files get huge (a 12-min 1080p export was already ~1.3 GB), and YouTube re-compresses anyway. **Note:** the *composition's* resolution isn't settable via the API (it needs the desktop app) — so don't fight it; control the **export** resolution here. A 4K composition exporting at 1080p is correct and normal.
...
After any import, `wait_for_job`; Descript auto-transcribes on import (don't re-transcribe). **Then CHECK the timeline actually got built** — `get_project` and confirm the composition's `duration` is the video's length, not `0`. A `0` means you omitted `add_compositions` (see the API shape above); fix it with a `prompt_project_agent` placement call, never by re-importing.
...
**LONG-FORM — ≤6 paid passes:** ① open-on-hook + filler words (verify the first 40s) · ② dead-air + bad takes (one reviewed destructive pass — hard gate; snapshot first if it's a Descript-only original) · ③ glossary corrections + Studio Sound ~55% + bare-min grade (one non-destructive call) · ④ B-roll placement (imports first) · ⑤ energy (punch-ins w/ swoosh + smooth transitions + SFX + contextual animations, key beats only) · ⑥ cards + emphasis pop-ups (hook + CTA + one every ~2–3 min + optional 1 stat — positions AND sizing specified: cap height ≈3.5% of frame height, panel ≥30% wider / ≥60% taller than the text, one short line each — see the 4K overflow trap in `graphics-style.md`). Chapters, title + description, and frame-QA are FREE Claude work. **Budget a 7th "text fix" pass on long-form** — text sizing usually needs one correction round after the first frame-QA; that's normal, not failure.
...
**ONE RENDER:** `publish_project` once — **always at `1080p`** (see the tool note above; never lower, 4K only on request) — on approval, and use that single export for BOTH the frame-QA and the agent's review link. Re-render only if a fix was actually needed.
...
1. Find/create the project; `import_media` the video **by URL** — pass `add_media` **and `add_compositions`** so the timeline is actually built; `wait_for_job` (auto-transcribes); then `get_project` to confirm the composition duration is non-zero.
...
3. **PASS ① — open on the hook + filler words:** cut everything before the confirmed hook line (the settling-in / "let's go" / throat-clearing intro), then remove filler words (conservative — ums/uhs, leave natural speech). **Say "CUT them — do NOT regenerate, overdub, or voice-clone any audio."** Left to itself Descript will "fix" a stutter by synthesizing the word in a cloned voice (seen live) — that puts AI-generated speech in the agent's own face-to-camera video, which is an authenticity problem for a realtor. Removal only. Verify the first 40s.
...
6. **PASS ④ — B-roll:** `editor-broll` sources + imports (**curated, never reused**, **~1 per 45–60 seconds** — a 10-min video ≈ 10–13 clips, a 15-min ≈ 15–20, ~10–15% of screen time), then ONE placement pass on the exact words — pick every moment from the transcript first (free) so all of them land in that single paid call. **Each clip must be instantly recognizable in a 4-second glance** — reject dark blurs and abstract macros. Horizontal b-roll on a vertical → **split-screen, never over-zoomed** (`${CLAUDE_PLUGIN_ROOT}/shared/layouts.md`).
...
8. **PASS ⑥ — cards + emphasis pop-ups (every position in the instruction):** a **hook card + a CTA card** on rounded brand panels + an **emphasis pop-up every ~2–3 min** (a key phrase from the transcript — a ~10-min video ≈ 3–5 pop-ups) + optionally ONE stat card — all **OFF THE FACE, in the LOWER THIRD (~75–80% down)** because long-form is 16:9 and the top is usually blocked by the speaker's head (`${CLAUDE_PLUGIN_ROOT}/shared/graphics-style.md` — state the exact zone in the instruction). **No word-by-word karaoke on long-form** (YouTube auto-captions cover accessibility); full captioning only if asked.
...
1. Find/import as above (by URL) — and if you imported, **pass `add_compositions` and then `get_project` to confirm the composition duration is NOT `0`** before editing. An empty timeline is the most common first-run failure; catching it here costs nothing, discovering it three passes in costs credits.
