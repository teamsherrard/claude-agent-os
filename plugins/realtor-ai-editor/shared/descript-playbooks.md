...
- **Studio Sound — clean the audio, but match the intensity to the SOURCE (its own step).** PHONE audio (most realtors) needs it: ~80–90% (100% for echoey rooms). A real external MIC is already clean — high-intensity Studio Sound then brightens/thins the voice (especially a deep voice) and sounds artificial, so go LIGHT (~30–40%) or OFF and judge on playback. If the agent has a separate high-quality mic recording, sync THAT in and skip Studio Sound entirely. Render-time effect — confirm on *playback*, not the scrub preview. Honest ceiling: a bad mic + reverberant room has limits → suggest a cheap lav mic at the source.
