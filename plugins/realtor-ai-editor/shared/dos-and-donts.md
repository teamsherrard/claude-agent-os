...
- DO put B-ROLL in every video — ALWAYS at least a little, even from Descript's built-in stock (zero setup). Never deliver a long-form with zero b-roll — but respect the caps: **max 3 clips on a short-form reel**; **long-form needs MORE — ~1 per 45–60 seconds (~10–13 on a 10-min video, ~10–15% of screen time)**, because a long talking head with sparse B-roll reads flat. Beyond that it's the agent's free manual 20% (see credit-tiers.md).
