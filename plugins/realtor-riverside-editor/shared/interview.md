...
4. **Audio** — `set_magic_audio` on EVERY speaker track (guests are usually on worse mics: 0.85 for them, 0.55 for the host on a good mic).
...
6. **Colour** — `set_color_correction` per speaker track; match the two looks toward each other rather than grading each to taste.
...
8. **Sections** — hand the topic-by-topic section map to the YouTube System for chapter names (`boundaries.md`).
