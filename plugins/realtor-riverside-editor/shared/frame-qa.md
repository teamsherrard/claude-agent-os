...
The connector never returns a frame. That does NOT mean the editor is blind — it means you climb a ladder, and you cite which rung you used. **The frame-read rule from `final-check.md` still stands: a visual PASS with no cited, actually-viewed frame is a FAIL.**

## The ladder (use the highest rung you can reach)

### Rung 1 — the exported file, read locally (best)
`exports_get_export` gives an S3 key, not a link, so the file reaches you when the agent downloads it from Riverside → it lands in their **Downloads** folder. In Code mode on their machine, that folder is readable. Then:
...
| Black bars / exposed edge after a punch-in | yes | yes | no |
...
Every frame you read costs tokens, not credits. A dozen well-chosen stills per edit is right; a frame-by-frame watch is not. Pick: the open, each card, each cutaway's midpoint, one punch-in peak, the CTA, the last frame.
