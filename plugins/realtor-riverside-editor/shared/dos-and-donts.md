...
- DO keep zooms gentle (1.06 normal, 1.10 strong, 1.14 once), keep good headroom, keep skin natural and flattering.
...
- DO turn Magic Audio on for every speaker track on every video. **0.70 default** (a decent mic in a normal room; 0.55 was inaudible on a real edit). **Phone / echoey rooms → 0.85**, 1.0 for a bad room. Only lower it if the audio is genuinely pristine AND full strength is thinning the voice.
...
- (Filler removal is a transcript-labelled cut/mute, never synthesis — nothing here ever clones a voice.)
...
- DO add *purposeful* energy: zooms on the emotional beats at the format's rate (about one a minute on long-form, one every 15 s on a reel), a size change on every jump cut, the hook banner, the CTA banners, a name strip on long-form, a music bed on every video. **Minimal ≠ bare** — a flat static talking head is *also* a bad edit.
...
- DON'T pile on random overlays. Riverside gives you few toys (no transitions, no SFX library) — that's a feature. Curate by video type (`effects-menu.md`).
...
- DO keep graphics purposeful: short-form a hook banner + the CTA banner (+ a pack banner on a spoken ask); long-form hook + name strip + pop-ups + pack banners + the primary CTA. Big, one line, animated in, OFF the face.
...
- DON'T let a zoom expose the frame edge — never below 1.0, keep it centred, verify on a frame (the step-back cuts OUT to 1.0, never past it). And DON'T claim a punch-in you have not read back: the keyframe route reports success and writes nothing; only the zoomed-scene route is real.
