...
- **Animated end card (`brand.assets.animated.end_card_*`)** — a full-frame MP4 from the Animated Content Kit. Full-frame needs no transparency, so it works today as a closing scene (`insert_media_as_scene` at the end, which extends the video by its length). Only the end card is ever animated as a file; banners, strips, pop-ups, and icons are PNGs animated by the timeline.
