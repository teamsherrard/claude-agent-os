...
2. **Read before you write.** Pull the Brain first (the persistence rule in `${CLAUDE_PLUGIN_ROOT}/shared/house-rules.md`), then `read_aligned_transcript` or a narrow `read_timeline_in_range` → keep the `revision`. Check the checkpoint log (`~/realtor-brain/editor/jobs/rs-<editId>.md`): if this craft is already logged, say so in one line and stop unless they want it changed.
...
6. **Log it.** One line in the checkpoint log: `<craft> · done · rev <n>`, then push the Brain (the persistence rule).
