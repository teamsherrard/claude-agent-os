...
- **`assets` — the Video Brand Kit** (designed once in the agent's Claude Design suite, stored in the workspace Drive at `02 · Brand/Video Brand Kit/` under canonical names). Setup registers the Drive paths (when Drive is on) AND gets each PNG into Riverside's Your Media — by uploading it, or by finding a file the agent dragged into the media panel themselves (`media_list_media`, matched on the canonical file name) — recording the media ids in `assets.riverside_media_ids` (`{"hook_9x16":"<24-hex>", …}`) — so an edit places a card with ONE `insert_overlay` call and no re-upload. The spec WINS over inferred values. When `assets` is populated, cards are **PLACED from these files, not built natively**. Empty `assets` → native cards.
