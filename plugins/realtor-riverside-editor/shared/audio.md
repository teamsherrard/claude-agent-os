...
|---|---|
| A decent mic in a normal room (most agents with a USB or lav mic) | **0.55** |
...
3. **Confirm it landed** — re-read the track: an `audio.enhancedAudioMix` keyframe with `enabled: true` and your level. If the write says unavailable or skipped, the recording is still processing: wait and say so. Never report clean audio that is not there.
