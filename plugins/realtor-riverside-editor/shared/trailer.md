...
- **Two formats from one build:** 9:16 for Stories and Reels, and 16:9 for the community post and X. Both are clones of the cut-only MASTER (`riverside-playbooks.md`), never of a finished edit and never of each other: an aspect change is refused once any overlay exists. Build the 9:16 first; the 16:9 is a second clone of the master with the same three keep-spans, cards in the lower third.
- **Cards:** the hook card for the first 2 s, then a closing card for the last 3 to 4 s that says when and where: *"Full video Thursday"* or *"Link in bio Thursday 9am"*. The date comes from the YouTube System's planner or the Short-Form System's calendar if either has it; if neither, ask once: *"When does the full video go live?"* The closing card is a NATIVE text card (`graphics-style.md`), because the kit's CTA banners carry baked words and cannot say the date; the kit's `link` variant may sit under it on the last 2 s if registered. No other pop-ups.
...
1. **Source.** The long-form's cut-only MASTER (logged as `master · <editId>` by the long-form line). If the long-form was never edited, run the reviewed clean cut on one edit and clone the master from it first; a trailer from a raw take carries the fillers into it.
...
3. **Build.** `clone_edit(masterEditId, title:"Trailer — <hook>")` → `update_aspect_ratio 9:16` FIRST (before any overlay exists) with the face centred → resolve the three keep-spans and cut everything else in ONE `cut_time_ranges` (head, the two gaps, the tail) → `remove_pauses 800` → captions in the brand look (`caption-style.md`, `fontSize 84`) → the hook card and the closing card (`graphics-style.md`, top-edge zone on 9:16) → the music bed at -18 dB (High intensity is fine here), placed twice if the track is shorter than the trailer.
...
5. **16:9 copy.** A second `clone_edit(masterEditId)`, the same three keep-spans in ONE `cut_time_ranges`, `remove_pauses 800`, the same captions and bed, both cards in the lower third, one frame check.
