...
| Listing photos — as motion stills | the actual property | no | media upload (JPG/PNG) → `insert_overlay` + Ken-Burns scale keyframes | yes | no |
...
5. **Placement & amount (HARD CAP)** — 2.5–5 s each, on the exact words; **full-cover is the DEFAULT** (after insert, scale the overlay track to the canvas — an unscaled overlay floats at its natural size in the centre); never over the hook, face, or CTA. Short-form (≤ ~2 min): **MAXIMUM 3.** Long-form: **~1 per 45–60 s** (a 10-min ≈ 10–13, a 15-min ≈ 15–20; ~10–15% of screen time) — the old 1-per-1.5–2-min rate tested flat on a real 12-min video. Place them all in ONE pass. **More than that is the agent's free 20%.**
...
8. **Licensing** — prefer no-attribution sources. If a CC-BY item is used, add attribution to the description or skip it.
...
All stock keys are optional and free. The agent can add Pexels/Pixabay/Unsplash/Coverr anytime in `riverside-setup`; more sources = more variety, never required to start.
