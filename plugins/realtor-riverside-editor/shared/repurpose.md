...
1. **The source is the cut-only MASTER, read once.** The long-form line clones it the moment the cut is locked (`master · <editId>` in the log); if there is no long-form edit yet, make ONE edit, run the reviewed clean cut, and clone the master from it. A clone of a FINISHED edit cannot become 9:16 (the connector refuses the aspect change once overlays exist). The master itself is never cut.
...
4. **Per clip, its own edit.** `clone_edit(masterEditId, title:"Reel N — <hook line>")`. On the clone: resolve the head (start → the clip's first word) and the tail (the clip's last word → end), plus any housekeeping inside the span, and apply them in ONE `cut_time_ranges`.
