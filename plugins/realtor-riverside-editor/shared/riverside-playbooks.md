...
4. **FINISH** — `set_color_correction` visible-but-natural → zooms on the beats, one per 12–20 s, plus a size change on each editorial cut (+ swoosh if present) → music bed at -18 dB (`insert_audio` maps the bed onto SOURCE time across the cuts; a 37 s library track covers 37 s of PLAYABLE reel — insert it again at `startMs:<track length>` to cover the rest) → hook card (first ~2 s, top-edge zone, `fontSize 72`, ≤25 chars) → CTA card (last ~4 s, same zone) → the logo with `insert_logo` if the kit has one (NOT `apply_brand` on a reel: it can also apply the kit's caption preset over the captions you just set). Diff, log, **ONE export at 1080p**, frame-QA, deliver. **Live-tested reel recipe (46 s from a 70 s raw intro): 18 revisions, ~12 min, zero credits; `create_edit_from_recording` was NOT gated.**
...
5. **PASS ③ — Magic Audio + grade:** `set_magic_audio` every speaker track at 0.70 (0.85 phone/echo) → `set_color_correction` on the speaker track (bare minimum). Read back the track; confirm both keyframes exist. Log.
...
7. **PASS ⑤ — energy:** the emotion-driven plan (`effects-menu.md`): zooms on the beats at one per 45–75 s (1.06 / 1.10 / 1.14 by strength), a step-back on the biggest reveal, a size change on every editorial cut, + swoosh if available. All the scene splits and slot settings in as few batches as possible. Read the scenes back; one frame per size. Log the count.
...
6. **PASS 4 — FINISH:** grade → 3–4 zooms on the beats (one per 12–20 s) → music bed -18 dB → hook banner (top zone, ~2 s) → CTA banner (on the CTA line to the end, top zone) → the logo with `insert_logo` if the kit has one (not `apply_brand` on a reel). Diff; log.
