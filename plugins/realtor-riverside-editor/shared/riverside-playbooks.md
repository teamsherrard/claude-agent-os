...
- `create_edit_from_recording(sessionId, projectId, title)` → `editId`. The recording is never modified. **May be feature-gated** (`FAILED_PRECONDITION`) — then: *"Open the recording in Riverside and hit Edit once — I'll take it from there,"* then `platform_list_edits(projectId)` and use the newest one. Don't retry the gated call.
...
**Graphics & text**
- **Brand-kit card (preferred when `brand.assets` is populated):** `insert_overlay(editId, assetId, startMs, durationMs, position:{x:{n,d},y:{n,d}}, expectedRevision)` — the PNG's media id comes from `brand.assets.riverside_media_ids` (registered at setup). Full-frame → position centre; the kit's reserved zones were designed off-face.
- **Native card:** `batch` op `insert_text_overlay {desiredInsertionTime:{n,d}, duration:{n,d}, name, trackZIndex:10, text, role:"TITLE", fontFamily, fontSize, color:"#FFFFFF", effect:"boxed", animation:[{name:"popIn",type:"in",duration:0.3,offset:0},{name:"popOut",type:"out",duration:0.25,offset:0}]}` → then `read_timeline_in_range` around that time to get the new text clip's `uniqueId` → `batch` op `modify_position {clipId, time:{n:0,d:1}, positionX, positionY, canvasWidth, canvasHeight}` to the format zone (`${CLAUDE_PLUGIN_ROOT}/shared/graphics-style.md`). One card = ONE text object. The `boxed` effect is the panel — its colour is Riverside's default (dark on white text); **you cannot brand the panel colour natively** (seen in the tool surface: no box-colour field) — which is exactly why the brand-kit PNG is preferred.
...
**Energy**
- Punch-in: `batch` ops `modify_scale {trackId:<speaker track>, time:t, scaleW:round(1.06·W), scaleH:round(1.06·H), canvasWidth:W, canvasHeight:H}` and `modify_scale {…, time:t+2.5s, scaleW:W, scaleH:H, …}`. Keep the position centred (a scaled track stays centred by default). **After the first punch-in, read the timeline and confirm the keyframes are there; verify on a frame that no edge shows** (`${CLAUDE_PLUGIN_ROOT}/shared/footage-look.md`). If a live run shows scale keyframes cutting rather than easing, keep the punch-ins as a snap-zoom (still fine, common on reels) and note it.
...
- `batch` ops `add_chapter {time:{n,d}, title}` — first at 0, ≥3 chapters, ≥10 s apart. Plus the `00:00`-format list in the description. **LIVE-TESTED: a Riverside-made edit already carries 3–4 AI-generated chapters** (read `chapters[]` in the timeline first) — keep the good ones, `move_chapter` a mis-timed one, add the missing ones; never stack a duplicate at the same beat.
...
Then: `platform_get_recording` → `project.id` + `studioId` → `create_edit_from_recording` (or the gated fallback) → `read_aligned_transcript`. If the transcript read returns `FAILED_PRECONDITION` with `transcript_load_failed`, it's still processing — wait a few minutes and try once more; `no_session_ids` means the edit has no recording behind it — wrong edit, pick again.
...
4. **FINISH** — `set_color_correction` bare-minimum → 2–3 punch-in keyframe pairs (+ swoosh if present) → music bed at -18 dB (`insert_audio` maps the bed onto SOURCE time across the cuts; a 37 s library track covers 37 s of PLAYABLE reel — insert it again at `startMs:<track length>` to cover the rest) → hook card (first ~2 s, top-edge zone, `fontSize 72`, ≤25 chars) → CTA card (last ~4 s, same zone) → `apply_brand` if the kit has a logo. Diff, log, **ONE export at 1080p**, frame-QA, deliver. **Live-tested reel recipe (46 s from a 70 s raw intro): 18 revisions, ~12 min, zero credits; `create_edit_from_recording` was NOT gated.**

**LONG-FORM — ≤6 passes:** ① open-on-hook + filler words (`cut_time_ranges` for the pre-hook + `remove_fillers Cut`; verify the first 40 s) · ② dead air + duplicate/bad takes (`remove_pauses 1500` + the resolved cuts — the reviewed destructive pass; snapshot first if the recording exists nowhere else) · ③ Magic Audio 0.55 + bare-min grade (two non-destructive calls) · ④ B-roll (~1 per 45–60 s, all placed in one go, each full-cover) · ⑤ energy (a couple of punch-in pairs on key beats + optional swoosh) · ⑥ text (lower third on first appearance + hook card + CTA + an emphasis pop-up every ~2–3 min, all in the LOWER THIRD zone on 16:9) + `add_chapter`s + `apply_brand`. Title/description/frame-QA are FREE Claude work. **Budget one "text fix" round** — sizing usually needs a correction after the first frame-QA; that's normal.
...
- On `CONFLICT`: STOP in this turn. Tell the agent plainly. On their yes, re-read and continue from the log.
...
6. **PASS ④ — B-roll:** `riverside-broll` picks every moment from the transcript first (free), then places them all: ~1 per 45–60 s, each 2.5–5 s, full-cover, unique, instantly recognisable, never over hook/face/CTA. Verify the count on the timeline. Log.
7. **PASS ⑤ — energy:** 2–3 punch-in keyframe pairs on key lines (1.06×, 2.5 s), + swoosh if available. Verify one on a frame. Log.
8. **PASS ⑥ — text + chapters + brand:** `add_lower_third` at the first on-camera moment (~5 s) → hook card (first ~2 s) → CTA card (last ~4 s) → emphasis pop-ups every ~2–3 min → `add_chapter`s → `apply_brand` (logo). **16:9 → everything in the LOWER THIRD (~75–80% down)**, one wide line each, sized per `graphics-style.md`. Compliance-check every word first. Diff; log.
9. **FREE:** title + description (chapters in `00:00` format) → **ONE export at 1080p** on approval → frame-QA (`frame-qa.md`) → fix + re-export only if a blocker fails → hand over as a review draft with the title/description → offer the music bed → offer to publish.
...
6. **PASS 4 — FINISH:** grade → 2–3 punch-ins → music bed -18 dB → hook card (top-edge zone, ~2 s) → CTA card (last ~4 s, above the captions) → logo via `apply_brand` if the kit has one. Diff; log.
7. **ONE export at 1080p** on approval → frame-QA → fix + re-export only on a blocker → review draft → offer to publish (`riverside-publish`).
