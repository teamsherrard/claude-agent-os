...
If Drive is on, look for the agent's **Video Brand Kit** in `02 · Brand/Video Brand Kit/` (canonical files, kit v3: `hook-16x9.png`, `hook-9x16.png`, `cta-16x9.png`, `cta-9x16.png`, `lower-third-16x9.png`, `emphasis-16x9.png`, `emphasis-9x16.png`, `end-screen.png`, the `icons/` folder, the eight-variant CTA pack `cta-{subscribe,comment,guide,call,book,dm,link,follow}-{16x9,9x16}.png` (registered into `brand.assets.cta_pack`), optional `lower-third-9x16.png`, `end-card-16x9.png`, `end-card-9x16.png`, `animated/end-card-*.mp4`, `logo-sting.mp4`, plus the `video-brand-spec` doc. `preview-16x9.png` and `preview-9x16.png` are review sheets: skip them). **Sanity-check the kit before registering:** a `hook-*`, `emphasis-*`, `lower-third-*`, or `cta-*` file that is fully opaque (no transparent pixels) is an old-style slate, not an overlay; say so in one line and point them at the Video Brand Kit v3 skill rather than placing a full-screen card over their face.
