...
Pull the Brain first (the persistence rule in `house-rules.md`; in Cowork the config only exists after the pull). If their **config exists** (setup is done), the **video is findable and ready**, and the ask matches a **standard job** (a reel · a long-form edit · a listing · clips-from-a-long-video), **skip ALL discovery questions**: run the connection preflight, say the one-line plan with the time estimate, get the single yes, and GO. The one-question rule is for when something is genuinely missing — not a step of every job.

## Which engine? (read this before anything else)

The Studio is the video editor. If an older config still says `default_engine: "descript"`, ignore it: the Descript editor is retired, and the Studio owns every edit. Set the key to `"riverside"` when you next save the config.
...
2. **Pull the Brain, then read the checkpoint log** at `~/realtor-brain/editor/jobs/rs-<editId>.md` (the persistence rule and the log convention in `${CLAUDE_PLUGIN_ROOT}/shared/house-rules.md`).
   ...
   - **Log missing** (it was never pushed to Drive, or Drive is not connected) → **Riverside remembers for you.** `editing_get_revision` → `editing_compare_revisions("1", <head>)`: the toggles tell you if fillers/pauses/Magic Audio ran; the created cuts and scenes tell you what else landed; `read_timeline_in_range` shows captions/overlays/cards. State what you believe is done and get a one-word confirm **before changing anything**. Then rebuild the log from what you found.
...
- Captions: karaoke on for shorts; long-form captions OFF (a mandatory pass, `caption-style.md`), with **emphasis pop-ups every ~2–3 min** + a name strip from Brand Graphics (`graphics-style.md`).
